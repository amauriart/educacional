document.addEventListener('DOMContentLoaded', function() {
    ...
    async function carregarJogos() {
        ...
        ultimaAtualizacao.textContent = 'Conectando à API...';
        ...
        try {
            ...
            if (data.totalJogos) {
                mensagem += ` | ${data.jogosComBetano} jogos com Betano`;
            }
            ultimaAtualizacao.textContent = mensagem;
            ...
            btnAtualizacao.textContent = '🔄 Atualizar Scanner';
        }
    }
    ...
    function exibirJogos(jogos, containerId, tipoOver) {
        ...
        if (!jogos || jogos.length === 0) {
            container.innerHTML = '<div class="vazio">⚠️ Nenhum jogo encontrado com os critérios para Over ' + tipoOver + ' gols.</div>';
            return;
        }
        ...
        container.innerHTML = jogos.map(jogo => `
            <div class="jogo-card">
                <div class="jogo-header">
                    ...
                    <span class="jogo-odds">Over ${tipoOver}: <strong>${jogo['oddsOver' + tipoOver.replace('.', '')]}</strong></span>
                </div>
                <div class="jogo-times">${jogo.times}</div>
                <div class="jogo-campeonato">${jogo.campeonato}</div>
                <div class="jogo-info">Betano • Próximas 24h</div>
            </div>
        `).join('');
    }
