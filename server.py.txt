...
# Em um cenário real, esta lista seria preenchida por uma API ou scraping
MOCK_GAMES = [
    {
        "time_start": "2025-11-27T23:30:00Z", # Em 2.5 horas
    ...
    {
        "time_start": "2025-11-28T10:00:00Z", # Em 12 horas
    ...
    {
        "time_start": "2025-11-29T18:00:00Z", # Em > 24 horas, será filtrado
    ...
    {
        "time_start": "2025-11-27T20:00:00Z", # Já começou, será filtrado
    ...
    {
        "time_start": "2025-11-28T12:00:00Z", # Em 14 horas
...
def process_games(games):
    ...
    for game in games:
        ...
        if start_time_utc > datetime.utcnow() and start_time_utc <= time_limit:
            
            # Formatação do horário para exibição
            # Convertendo para o fuso horário de Brasília (UTC-3) apenas para exibição
            start_time_brt = start_time_utc - timedelta(hours=3)
            time_display = start_time_brt.strftime("%H:%M") # Ex: 20:30

            # Cria a estrutura de exibição, conforme Requisito 7
            base_data = {
    ...
    # 1. Agrupa por tipo (0.5 primeiro, depois 1.5)
    # 2. Ordena dentro de cada grupo pela maior odd (decrescente)
    
    # Separa os resultados por mercado para ordenação
